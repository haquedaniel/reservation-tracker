...
def load_listings_lookup(input_path: Path) -> pd.DataFrame:
    """
    Load the listing lookup table and standardise its column names.
...
def expand_expenses(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert the Expenses sheet from wide monthly format to long monthly format.
    """
    df = df.copy()
    df.columns = df.columns.str.strip()
    ...
    df_long["expense_type"] = df_long["Type"] if "Type" in df_long.columns else pd.NA
    df_long["expense_type"] = (
        df_long["expense_type"]
        .astype(str)
        .str.strip()
        .str.lower()
...
def olddataframe_to_html_table(
    df: pd.DataFrame,
    columns: list[str] | None = None,
    styled: bool = True
) -> str:
    ...
    - columns: optional subset/order of columns
    - styled: whether to apply nicer formatting (default True)
    """
...
def build_revenue_this_week(
    ...
    for _, row in week.iterrows():
        # Only allocate expenses that are intended to vary with occupied nights.
        # Fixed monthly costs such as mortgage or insurance should not be
        # charged to an individual reservation here.
        stay_expenses = expenses[
            ...
            (expenses["expense_type"] == "estimated per night")
        ]
...
def main() -> None:
   ...
   # weekly = weekly.rename(columns={"Name": "listing_name"})
    ...
    # -----------------------------
    if True : # mode == "weekly-email":
        summary = build_weekly_arrivals_departures_summary(df_reservations)
        weekly_occupancy_html = dataframe_to_html_table(weekly_occupancy_pivot)
